...
# Publishes the zvec library to Maven repository (S3).
# Usage:
#   ./publish.sh               # Release publish (checks if exists)
#   SNAPSHOT=true ./publish.sh  # Snapshot publish (appends -SNAPSHOT)
...
if [ "$SNAPSHOT" = "true" ]; then
  ...
  ./gradlew lib-zvec:publishMavenPublicationToSeqeraRepository -Pversion=$VERSION
else
  ...
  ret=$(aws s3api head-object --bucket $BUCKET --key $KEY 2>&1) && {
    ...
    echo "NOTE: Library $NAME-$VERSION already exists - skipping publishing"
  } || {
    if [[ $ret == *"Not Found"* ]]; then
      # the lib does not exist => publish it
      ./gradlew lib-zvec:publishMavenPublicationToSeqeraRepository
    else
